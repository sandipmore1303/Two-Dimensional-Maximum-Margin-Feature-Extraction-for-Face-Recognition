...
{ public  static int nr,nc;
 ...
 System.out.println("Noofclasses="+NoofClasses);
  ...
  f=new File("C:\\test.jpg");
  try{
  image2 = ImageIO.read(f); 
  } catch(IOException e){System.out.println("Cant read file C:\\test.jpg");}
        ...
        if(!children[NoofFiles].equals("Thumbs.db"))
         {
          ...
          for(j=0;j<IMAGE_HEIGHT;j++)
          {
           ...
           blue =((rgb ) & 0xff); 
            try  {	                                           
        	      Mi[i][j][0][current_class] += (alpha)*(1.0); 
                  Mi[i][j][1][current_class] += (red)*(1.0); 
                  Mi[i][j][2][current_class] +=(green)*(1.0); 
                  Mi[i][j][3][current_class] +=(blue)*(1.0); 
  ...
  public static void step2_2D2MMC() throws IOException
{
  ...
  public static void step2_2D2MMC_compute_Wmmc_r(){
    ...
    //using INDEX array form the Wmmc_r by taking first nr eigen vectors corresponding to first nr values of x array
    //of x array
      ...
      for(int NoofFiles=0;NoofFiles<children.length;NoofFiles++)
      {
       ...
       try{
        ...
        if(!children[NoofFiles].equals("Thumbs.db"))
         {
          ...
          for(j=0;j<IMAGE_HEIGHT;j++)
          {
           ...
           green =((rgb >> 8) & 0xff); 
           blue =((rgb ) & 0xff); 
            try  {X0.set(i,j,1.0*alpha);   
                  X1.set(i,j,1.0*red);   
                  X2.set(i,j,1.0*green);   
                  X3.set(i,j,1.0*blue);   
        	      }  catch(ArrayIndexOutOfBoundsException e){ } 
           } 
        ...
        //save 2DMMC into 2DMMC databse
          ...
          for(j=0;j<nr;j++)
          {
          	alpha=(int)_2D2MMC0.get(i,j);
          	red=(int)_2D2MMC1.get(i,j);
          	green=(int)_2D2MMC2.get(i,j);
          	blue=(int)_2D2MMC3.get(i,j);
          ...
          ImageIO.write( image3, "jpg" /* "png" "jpeg" ... format desired */,
               f3 /* target */ );
         ...
         for(i=0;i<nc*nr;i++)
           {
          	alpha=(int)_1DMMC0[fileno][i];
          	red=(int)_1DMMC1[fileno][i];
          	green=(int)_1DMMC2[fileno][i];
          	blue=(int)_1DMMC3[fileno][i];
          	alpha = ((alpha << 24) & 0xff000000); 
            red = ((red << 16) & 0x00ff0000); 
            green = ((green << 8) & 0x0000ff00); 
            blue = ((blue ) & 0x000000ff); 
          ...
          ImageIO.write( image4, "jpg" /* "png" "jpeg" ... format desired */,
               f4 /* target */ );           	
        ...
        if(!children[NoofFiles].equals("Thumbs.db"))
         {
          ...
          
          {
           ...
           blue =((rgb ) & 0xff); 
            try  {	                                           
        	      SMi0[i][current_class] += (alpha)*(1.0); 
                  SMi1[i][current_class] += (red)*(1.0); 
                  SMi2[i][current_class] +=(green)*(1.0); 
                  SMi3[i][current_class]+=(blue)*(1.0); 
                  //System.out.println(" i="+i+" j= "+j+"current_class="+current_class); 
                  //System.out.println(Mi[i][j][0][current_class]+" "+Mi[i][j][1][current_class]+" "+Mi[i][j][2][current_class]+" "+Mi[i][j][3][current_class]);
                  }  catch(ArrayIndexOutOfBoundsException e){ } 
          }      	  	 
